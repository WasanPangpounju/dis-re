...
interface Service { id: string; icon: string; title: string; description: string; features: string[] }
  ...
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <h1 style={{ fontSize: '26px', fontWeight: 700, color: '#111827', fontFamily: 'var(--font-kanit)' }}>บริการ</h1>
        ...
        {items.map(item => (
          <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '16px 20px', borderBottom: '1px solid #f3f4f6' }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '13px', color: '#6b7280', marginTop: '2px' }}>{item.features.length} features</div>
            </div>
      ...
      {deleteId && <ConfirmDialog message="ต้องการลบบริการนี้ใช่หรือไม่?" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} />}
