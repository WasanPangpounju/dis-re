...
export default function PortfolioEditPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/portfolio/${id}`).then(r => r.json()).then(d => { setForm(d); setLoading(false) })
  }, [id])
  ...
  async function handleSave() {
    setSaving(true)
    try {
      const payload = { ...form, categoryLabel: CATEGORY_LABELS[form.category] || form.categoryLabel }
      const res = await fetch(`/api/admin/portfolio/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
  ...
  return (
    <div style={{ maxWidth: '700px' }}>
      <h1 style={{ fontSize: '26px', fontWeight: 700, color: '#111827', marginBottom: '24px', fontFamily: 'var(--font-kanit)' }}>แก้ไขผลงาน</h1>
      ...
      <AdminCard>
        ...
        <AdminSelect label="หมวดหมู่" value={form.category || 'research'} onChange={v => set('category', v)} options={CATEGORIES} />
        ...
        <AdminSelect label="สีพื้นหลัง" value={form.bgColor || 'from-teal-50 to-teal-100'} onChange={v => set('bgColor', v)} options={BG_COLORS} />
